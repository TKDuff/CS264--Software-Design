...
class Canvas
{
   ...
   //Method to export file to svg
    ...
    public void styling()
    {       //Cannot change style of an empty stack with no shapes
            if(Shapes.Count == 0 ){
                WriteLine("\nWARNING - Cannot change style of an empty canvas\n");
            } else {
            Write("Enter the z-index of the shape who style you want to change: ");     //User selects the shape using the z-index
            ...
            switch (input)
            {
                ...
                Shapes[i].stroke = ReadLine();
                break;
                case "fill":
                Write("Enter the new fill colour: ");
                Shapes[i].fill = ReadLine();
                break;
                case "strokeW":
                Write("Enter the new stroke width (number): ");
                Shapes[i].strokeWidth = ReadLine();
                break;
                default:
